...
public class RepoResource {
    ...
    @RequestMapping(value = "/_search/repos",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Repo>> searchRepos(@RequestParam String query, Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to search for a page of Repos for query {}", query);
        Page<Repo> page = repoService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/repos");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
